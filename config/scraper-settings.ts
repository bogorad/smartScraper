...
const scraperSettings: ScraperSettings = {
  maxLlmRetries: process.env.MAX_LLM_RETRIES
    ? parseInt(process.env.MAX_LLM_RETRIES, 10)
    : 2,
  minXpathScoreThreshold: process.env.MIN_XPATH_SCORE_THRESHOLD
    ? parseFloat(process.env.MIN_XPATH_SCORE_THRESHOLD)
    : 10,
  domComparisonThreshold: process.env.DOM_COMPARISON_THRESHOLD
    ? parseFloat(process.env.DOM_COMPARISON_THRESHOLD)
    : 0.6,
  knownSitesStoragePath:
    process.env.KNOWN_SITES_STORAGE_PATH ||
    path.resolve(__dirname, "..", "data", "known_sites_storage.json"),
  ...
  puppeteerHeadless:
    process.env.PUPPETEER_HEADLESS === "false"
      ...
      : process.env.PUPPETEER_HEADLESS === "new"
        ? "new"
        : true,
  puppeteerDefaultTimeout: process.env.PUPPETEER_TIMEOUT
    ? parseInt(process.env.PUPPETEER_TIMEOUT, 10)
    : 30000,
  puppeteerNavigationTimeout: process.env.PUPPETEER_NAV_TIMEOUT
    ? parseInt(process.env.PUPPETEER_NAV_TIMEOUT, 10)
    : 20000,
  puppeteerLaunchTimeout: process.env.PUPPETEER_LAUNCH_TIMEOUT
    ? parseInt(process.env.PUPPETEER_LAUNCH_TIMEOUT, 10)
    : 20000,
  puppeteerPostLoadDelay: process.env.PUPPETEER_POST_LOAD_DELAY
    ? parseInt(process.env.PUPPETEER_POST_LOAD_DELAY, 10)
    : 2000,
  puppeteerInteractionDelay: process.env.PUPPETEER_INTERACTION_DELAY
    ? parseInt(process.env.PUPPETEER_INTERACTION_DELAY, 10)
    : 1000,
  puppeteerViewport: {
    width: process.env.PUPPETEER_VIEWPORT_WIDTH
      ? parseInt(process.env.PUPPETEER_VIEWPORT_WIDTH, 10)
      : 1920,
    height: process.env.PUPPETEER_VIEWPORT_HEIGHT
      ? parseInt(process.env.PUPPETEER_VIEWPORT_HEIGHT, 10)
      : 1080,
    deviceScaleFactor: 1,
    isMobile: false,
    hasTouch: false,
  ...
  domStructureMaxTextLength: process.env.DOM_STRUCTURE_MAX_TEXT_LENGTH
    ? parseInt(process.env.DOM_STRUCTURE_MAX_TEXT_LENGTH, 10)
    : 15,
  domStructureMinTextSizeToAnnotate: process.env
    .DOM_STRUCTURE_MIN_TEXT_SIZE_TO_ANNOTATE
    ? parseInt(process.env.DOM_STRUCTURE_MIN_TEXT_SIZE_TO_ANNOTATE, 10)
    : 100,
  defaultUserAgent:
  ...
  contentIdKeywordsRegex:
    /^(article|post|story|main)[-_]?(content|body|text|area|container|wrapper)$/i,
  contentClassKeywordsRegex:
    /^(article|post|story|entry|blog|main|text|news)[-_]?(content|body|text|area|container|wrapper|inner|item|element)|paywall|dropcap/i,
  curlTimeout: process.env.CURL_TIMEOUT
    ? parseInt(process.env.CURL_TIMEOUT, 10)
    : 30000,
};
