...
import { getQueueStats, workerEvents } from '../../core/engine.js';
...
function renderWorkersHtml(stats: { active: number; max: number; activeUrls: string[] }): string {
  const list = stats.activeUrls.length > 0
    ? `<ul style="margin: 0; padding-left: 16px;">${stats.activeUrls.map(url => 
...
function broadcast(data: { active: number; max: number; activeUrls: string[] }) {
  ...
  for (const client of clients) {
    try {
      ...
      logger.debug(`[SSE] Client disconnected, removing from broadcast list`);
...
dashboardRouter.get('/events', async (c) => {
  ...
  const stream = new ReadableStream({
    start(controller) {
      ...
      controller.enqueue(new TextEncoder().encode(`event: workers\ndata: ${html}\n\n`));
      ...
      if (clientId) {
        for (const client of clients) {
          if (client.id === clientId) {
            clients.delete(client);
            logger.debug(`[SSE] Client disconnected, total clients: ${clients.size}`);
            break;
          }
        }
      }
  ...
  return c.html(
    <Layout title="Dashboard - SmartScraper" activePath="/dashboard" theme={theme}>
      ...
      <div class="stats-grid" style="margin-bottom: 12px;">
        <StatsCard title="Total" value={stats.scrapeTotal} />
      ...
      <div class="card" style="margin-bottom: 16px;" hx-ext="sse" sse-connect="/dashboard/events" sse-swap="workers">
        <div class="card-header">{queueStats.active} of {queueStats.max} Workers</div>
        {queueStats.activeUrls.length > 0 ? (
          <ul style="margin: 0; padding-left: 16px;">
            {queueStats.activeUrls.map(url => (
              <li class="code text-sm" style="word-break: break-all;">{url}</li>
            ))}
          </ul>
        ) : (
          <p class="text-muted text-sm" style="margin: 0;">No active scrapes</p>
