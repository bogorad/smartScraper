...
describe('TwoCaptchaAdapter', () => {
  ...
  describe('solveIfPresent', () => {
    ...
    it('should return unsupported for generic CAPTCHA without submitting to solver', async () => {
      ...
      expect(result).toEqual({
        solved: false,
        reason: 'Generic CAPTCHA solving is unsupported in this pass'
      });
  ...
  describe('unsupported CAPTCHA types', () => {
    it.each([
      [CAPTCHA_TYPES.GENERIC, 'Generic CAPTCHA solving is unsupported in this pass'],
      [CAPTCHA_TYPES.CLOUDFLARE, 'Turnstile CAPTCHA solving is unsupported in this pass'],
      ['turnstile', 'Turnstile CAPTCHA solving is unsupported in this pass'],
      ['recaptcha', 'reCAPTCHA CAPTCHA solving is unsupported in this pass'],
      ['hcaptcha', 'hCaptcha CAPTCHA solving is unsupported in this pass']
    ] as const)('should reject %s without submitting to solver', async (captchaTypeHint, reason) => {
      const result = await adapter.solveIfPresent({
        pageId: 'page-123',
        pageUrl: 'https://example.com',
        captchaTypeHint,
