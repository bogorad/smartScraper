...
func (t *TmuxSession) SendCommand(command string, verbose bool) error {
	...
	cmd := exec.Command("tmux", "-S", t.Socket, "send-keys", "-t", t.Name+":0.0", "--", command, "Enter")
	output, err := cmd.CombinedOutput()
	if err != nil {
...
func (t *TmuxSession) CapturePaneOutput(lines int) (string, error) {
	...
	startLine := -lines
	cmd := exec.Command("tmux", "-S", t.Socket, "capture-pane", "-t", t.Name+":0.0", "-p", "-S", strconv.Itoa(startLine))
	output, err := cmd.CombinedOutput()
	if err != nil {
